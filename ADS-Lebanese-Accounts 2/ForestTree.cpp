...
bool ForestTree::applyTransaction(int accountNumber, Transaction *transaction) {
  ...
  accountNode->data.addTransaction(
      transaction); // Dereference pointer to pass the object
...
void ForestTree::removeTransaction(const int &accountNumber,
  ...
  if (account) {
    ...
    cout << "Transaction with ID " << transactionId
         << " removed from the account " << accountNumber << endl;

    // Now update the parent balances
    cout << "Updating parent balances after transaction removal." << endl;
    updateParentBalances(
        accountNumber); // Pass the account node itself for parent updates
  } else {
    cout << "Account with the number " << accountNumber << " is not found"
         << endl;
  }
}
...
bool ForestTree::deleteAccount(int accountNumber) {
  ...
  if (root != nullptr && root->data.getAccountNumber() == accountNumber) {
    std::cout << "Account is the root. Deleting root account " << accountNumber
              << std::endl;
  ...
  // subtrees
  std::cout << "Searching and deleting account " << accountNumber
            << " in the tree." << std::endl;
  nodeToDelete = findNode(root, accountNumber);

  if (nodeToDelete != nullptr) {
    nodeToDelete->data.setBalance(0); // Reset the balance to 0
    ...
    std::cout << "Account " << accountNumber
              << " deleted successfully, now updating parent balances."
              << std::endl;

    return true;
  }

  std::cout << "Account " << accountNumber << " not found, deletion failed."
            << std::endl;
...
void ForestTree::updateParentBalances(FNodePtr node) {
  std::cout
      << "Updating parent balances starting from node with account number: "
      << node->data.getAccountNumber() << std::endl;
  ...
  while (node != nullptr) {
    ...
    if (parent == nullptr) {
      std::cerr << "No valid parent found for account number "
                << node->data.getAccountNumber() << std::endl;
...
void ForestTree::updateParentBalances(const int &accountNumber) {
  ...
  if (node == nullptr) {
    std::cerr << "Account with number " << accountNumber << " not found!"
              << std::endl;
    return;
  }
  ...
  while (node != nullptr) {
    ...
    // if no parent and the account is not on the 1st level
    if (parent == nullptr) {
      //cerr << "No valid parent found for account number "
          // << node->data.getAccountNumber() << std::endl;
      break; // Stop when reaching a node with no parent (root level)
    }
...
void ForestTree::updateNodeBalance(FNodePtr current, int accountNumber,
                                   int delta) {
  // Step 1: Find the parent of the current node
  FNodePtr parent = findParent(root, accountNumber,
                               0); // Assuming root is your tree's root pointer

  if (parent != nullptr) {
    // Step 2: Update the parent balances
    updateParentBalances(accountNumber);
  } else {
    std::cerr << "Parent not found for account number " << accountNumber
              << std::endl;
  }
}
...
void ForestTree::generateReportFile(ForestTree::FNodePtr accountNode) const {
  ...
  string fileName = "/Users/tonydawra/Downloads/DS-Lebanese-Accounts 2/accounts/account_" +
  ...
  const auto &transactions =
      accountNode->data.getTransactions(); // Assuming this returns a
                                           // std::vector<Transaction*>

  if (!transactions.empty()) {
    for (const auto *transaction :
         transactions) { // Iterate through the vector of Transaction pointers
      reportFile << "Transaction ID: " << transaction->getTransactionId()
                 << endl;
      reportFile << "Type           : "
                 << (transaction->getTransactionType() == 'D' ? "Debit"
                                                              : "Credit")
                 << endl;
      reportFile << "Amount         : $" << fixed << setprecision(2)
                 << transaction->getTransactionAmount() << endl;
      reportFile << "------------------------------------------" << endl;
    }
  } else {
